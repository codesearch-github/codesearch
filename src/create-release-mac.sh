...
TOMCAT_URL="http://mirror.cc.columbia.edu/pub/software/apache/tomcat/tomcat-7/v7.0.54/bin/apache-tomcat-7.0.54.tar.gz"
...
DOCFILES="$(greadlink -f $README) $(greadlink -f $AUTHORS) $(greadlink -f $COPYING)"
CONFIG=$(greadlink -f $CONFIG)
SEARCHER=$(greadlink -f $SEARCHER)
INDEXER=$(greadlink -f $INDEXER)
...
cd ..
tar -czf $TOMCAT_BUNDLE_NAME.tar.gz $TOMCAT_BUNDLE_NAME
