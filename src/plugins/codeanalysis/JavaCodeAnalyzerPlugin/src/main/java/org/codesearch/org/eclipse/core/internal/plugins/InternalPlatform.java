...
import java.net.URL;
import org.codesearch.org.eclipse.core.internal.model.RegistryLoader;
import org.codesearch.org.eclipse.core.runtime.*;
import org.codesearch.org.eclipse.core.runtime.model.Factory;
import org.codesearch.org.eclipse.core.runtime.model.PluginRegistryModel;
...
public class InternalPlatform {
	...
	public static void installPlugins(URL[] installURLs) throws CoreException {
		...
		BundleContext context = org.codesearch.org.eclipse.core.internal.runtime.InternalPlatform.getDefault().getBundleContext();
		for (int i = 0; i < installURLs.length; i++) {
			try {
				context.installBundle(installURLs[i].toExternalForm());
			} catch (BundleException e) {
				IStatus status = new Status(IStatus.ERROR, Platform.PI_RUNTIME, 0, org.codesearch.org.eclipse.core.internal.plugins.Policy.bind("platform.cannotInstallPlugin", installURLs[i].toExternalForm()), e); //$NON-NLS-1$
				result.merge(status);
			}
		}
		if (!result.isOK())
