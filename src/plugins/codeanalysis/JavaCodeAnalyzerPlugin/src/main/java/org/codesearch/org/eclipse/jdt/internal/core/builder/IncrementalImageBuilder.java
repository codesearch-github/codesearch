...
import org.codesearch.org.eclipse.jdt.internal.compiler.classfmt.*;
...
protected boolean checkForClassFileChanges(IResourceDelta binaryDelta, ClasspathMultiDirectory md, int segmentCount) throws CoreException {
	...
	switch(resource.getType()) {
		case IResource.FOLDER :
			...
			return true;
		case IResource.FILE :
			if (!isExcluded && org.codesearch.org.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resource.getName())) {
...
protected void findAffectedSourceFiles(IResourceDelta binaryDelta, int segmentCount, StringSet structurallyChangedTypes) {
	...
	switch(resource.getType()) {
		case IResource.FOLDER :
			...
			return;
		case IResource.FILE :
			if (org.codesearch.org.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resource.getName())) {
				IPath typePath = resource.getFullPath().removeFirstSegments(segmentCount).removeFileExtension();
				switch (binaryDelta.getKind()) {
					case IResourceDelta.ADDED :
					case IResourceDelta.REMOVED :
...
protected boolean findSourceFiles(IResourceDelta sourceDelta, ClasspathMultiDirectory md, int segmentCount) throws CoreException {
	...
	switch(resource.getType()) {
		...
		case IResource.FILE :
			...
			if (org.codesearch.org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resourceName)) {
				IPath typePath = resource.getFullPath().removeFirstSegments(segmentCount).removeFileExtension();
				...
				return true;
			} else if (org.codesearch.org.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resourceName)) {
				// perform full build if a managed class file has been changed
...
/**
 * @see org.codesearch.org.eclipse.jdt.internal.core.builder.AbstractImageBuilder#writeClassFileContents(org.codesearch.org.eclipse.jdt.internal.compiler.ClassFile, org.codesearch.org.eclipse.core.resources.IFile, java.lang.String, boolean, org.codesearch.org.eclipse.jdt.internal.core.builder.SourceFile)
 */
