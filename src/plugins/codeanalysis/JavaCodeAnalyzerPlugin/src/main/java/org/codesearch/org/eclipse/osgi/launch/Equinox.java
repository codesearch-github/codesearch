 ...
 *******************************************************************************/
package org.codesearch.org.eclipse.osgi.launch;
...
public class Equinox implements Framework {
	private static final String implName = "org.codesearch.org.eclipse.osgi.framework.internal.core.EquinoxLauncher"; //$NON-NLS-1$
	/**@GuardedBy this*/
		...
		for (int i = 0; i < cp.length; i++) {
			...
			try {
				if (cp[i].getFile().endsWith("/")) { //$NON-NLS-1$
					manifest = new URL(cp[i], org.codesearch.org.eclipse.osgi.framework.internal.core.Constants.OSGI_BUNDLE_MANIFEST).openStream();
				} else {
					manifest = new URL("jar:" + cp[i].toExternalForm() + "!/" + org.codesearch.org.eclipse.osgi.framework.internal.core.Constants.OSGI_BUNDLE_MANIFEST).openStream(); //$NON-NLS-1$ //$NON-NLS-2$
				}
				...
				if (bsnSpec == null)
					continue;
				String internalBSN = org.codesearch.org.eclipse.osgi.framework.internal.core.Constants.getInternalSymbolicName();
