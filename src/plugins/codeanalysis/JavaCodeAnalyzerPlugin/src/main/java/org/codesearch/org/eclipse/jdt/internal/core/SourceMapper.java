...
import org.codesearch.org.eclipse.jdt.core.IField;
 ...
 *
 * @see org.codesearch.org.eclipse.jdt.internal.core.JarPackageFragment
 */
public class SourceMapper
			...
			try {
				...
				for (Enumeration entries = zip.entries(); entries.hasMoreElements(); ) {
					...
					if (!entry.isDirectory()) {
						...
						} else if (!containsJavaSource && org.codesearch.org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(entryName)) {
					...
					try {
						...
						for (int i = 0, max = members.length; i < max; i++) {
							...
							if (member.getType() == IResource.FOLDER) {
								...
								containsADefaultPackage = true;
							} else if (!containsJavaSource && org.codesearch.org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resourceName)) {
								containsJavaSource = true;
							}
						}
					} catch (CoreException e) {
			...
			if (target instanceof IContainer) {
				...
				try {
					...
					for (Enumeration entries = zip.entries(); entries.hasMoreElements(); ) {
						ZipEntry entry = (ZipEntry) entries.nextElement();
						String entryName;
						if (!entry.isDirectory() && org.codesearch.org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(entryName = entry.getName())) {
		...
		try {
			...
			for (int i = 0, max = resources.length; i < max; i++) {
				...
				if (i == max - 1 && hasDefaultPackage) {
					...
					for (int j = 0; j < max; j++) {
						if (org.codesearch.org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resources[i].getName())) {
	...
	public char[] findSource(IType type, String simpleSourceFileName) {
		...
		String name = org.codesearch.org.eclipse.jdt.internal.core.util.Util.concatWith(pkgFrag.names, simpleSourceFileName, '/');
	...
	public char[] findSource(String fullName) {
		...
		if (target instanceof IContainer) {
			...
			if (res instanceof IFile) {
				try {
					source = org.codesearch.org.eclipse.jdt.internal.core.util.Util.getResourceContentsAsCharArray((IFile)res);
