...
public class IndexingJob implements Job {
	...
	@Inject
	public IndexingJob(ConfigurationReader configReader, DBAccess dba,
			PluginLoader pluginLoader) {
		this.configReader = configReader;
		this.dba = dba;
		this.pluginLoader = pluginLoader;
		indexLocation = configReader
				.getValue(XmlConfigurationReaderConstants.INDEX_LOCATION);
	}
	 ...
	 * 
	 * @param jec
	 *            the JobExecutionContext containing the tasks, the id of the
	 *            job and whether the job is flagged as terminated or not
	 * @throws JobExecutionException
	 *             if the execution of a task was not successful or if the job
	 *             was terminated
	 */
	@Override
	public void execute(JobExecutionContext jec) throws JobExecutionException {
		...
		LOG.info("Executing " + jec.getJobDetail().getKey().toString()
				+ ", indexing " + repositories.size() + " repositories");
		...
		try {
			...
			Task indexingTask = new IndexingTask(
					dba,
					pluginLoader,
					configReader
							.getValue(XmlConfigurationReaderConstants.SEARCHER_LOCATION));
			indexingTask.setRepositories(repositories);
			indexingTask.setIndexLocation(configReader
					.getValue(XmlConfigurationReaderConstants.INDEX_LOCATION));
			indexingTask.setJob(this);
			indexingTask.execute();
		} catch (TaskExecutionException ex) {
			throw new JobExecutionException(
					"Execution of IndexingJob threw an exception" + ex);
		}

		LOG.debug("Finished execution of job in "
				+ (new Date().getTime() - startDate.getTime()) / 1000f
				+ " seconds");
		LOG.info("Finished execution of "
